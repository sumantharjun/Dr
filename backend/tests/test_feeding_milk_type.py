...
def test_milk_type_is_required(client, auth):
    headers, _ = auth()
    r = client.post(
        "/feeding/logs",
        headers=headers,
        json={"milk_consumed_ml": 100, "method": "manual"},
    )
...
def test_all_milk_types_accepted_and_round_trip(client, auth, milk_type):
    headers, _ = auth()
    r = client.post(
        "/feeding/logs",
        headers=headers,
        json={"milk_consumed_ml": 100, "method": "manual", "milk_type": milk_type},
    )
...
def test_unknown_milk_type_rejected(client, auth):
    headers, _ = auth()
    r = client.post(
        "/feeding/logs",
        headers=headers,
        json={"milk_consumed_ml": 100, "method": "manual", "milk_type": "goat_milk"},
    )
    assert r.status_code == 422, r.text


def test_device_report_omits_milk_type(client, auth, make_device):
...
def test_device_report_accepts_milk_type(client, auth, make_device):
    headers, _ = auth()
    _, api_key = make_device(headers)
    r = client.post(
        "/feeding/device-report",
...
def test_device_report_rejects_bad_milk_type(client, auth, make_device):
    headers, _ = auth()
    _, api_key = make_device(headers)
    r = client.post(
        "/feeding/device-report",
