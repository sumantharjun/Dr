...
def test_cannot_log_feed_for_another_users_device(client, auth, make_device, make_baby):
...
def test_can_log_feed_for_own_device(client, auth, make_device, make_baby):
    headers, _ = auth()
...
def test_manual_feed_without_device_allowed(client, auth, make_baby):
    headers, _ = auth()
    baby_id = make_baby(headers)
    r = client.post(
        "/feeding/logs",
        headers=headers,
        json={"baby_id": baby_id, "milk_consumed_ml": 90, "method": "manual",
